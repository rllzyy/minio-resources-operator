...
func (r *ReconcileMinioBucket) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	minioClient, err := minio.New(config.RuntimeServerConfiguration.GetHostname(), config.RuntimeServerConfiguration.AccessKey, config.RuntimeServerConfiguration.SecretKey, config.RuntimeServerConfiguration.SSL)
	if err != nil {
